...
%\paragraph{Endophenotypes/Biomarkers of schizophrenia}
...
% Table \ref{Tab:Experiments} summarizes the experimental observations we want to include in the framework presented here. This directly tells us how to define the capabilities
% which our models have to produce. A model within our framework must be able to produce each of the five capabilities included in Table \ref{Tab:Experiments}. Accordingly,
% our capabilities file comprises five classes, each defining one of the capabilities. However, note that the experimental observations summarized in Table \ref{Tab:Experiments}
% describe the difference between healthy control subjects and schizophrenic patients. Therefore, the capabilities do not merely require the model to produce power at $x$\,Hz
% in response to $y$\,Hz drive, the model has to produce this power for two different versions of the model, one representing the healthy control network and one representing
% the 'schizophrenia-like' network. 
% 
% 
% \subsubsection{Model(s)}
% 
% 
% \subsubsection{Score(s)}
...
% 
% \subsubsection{Tests/Testsuites}
% The five tests we have implemented, test the five observations summarized in Table \ref{Tab:Experiments} individually. Additionally, we have implemented two testsuites which combine some of
% the individual observations. The first one comprises the tests for the fundamental frequencies at the three drive frequencies. The second one, additionally tests the (sub)harmonics for $20$ and $40$\,Hz, respectively. 
% 
% 
...
{\includegraphics[width=0.7\textwidth]{Figures/Example_showcase_plus_methods_03}}
...
\includepdf[pages={1-9}]{Code/Example_Experimental_Database.pdf}
\includepdf[pages={1-3}]{Code/Example_Model_Comparison2.pdf}
\includepdf[pages={1-2}]{Code/Example_Prediction.pdf}
\includepdf[pages={1-3}]{Code/Example_showcase_plus_methods.pdf}
