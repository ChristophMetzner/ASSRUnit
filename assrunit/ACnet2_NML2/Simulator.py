...
class Simulation(object):
	...
	def __init__(self, 
 	...
 	def go(self):
		...
		pynml.print_comment_v("Running a simulation of %s ms with timestep %s ms: %s"%(self.sim_time, self.dt, self.lems_file))
		...
		if self.simulator == 'jNeuroML':
		    results = pynml.run_lems_with_jneuroml(self.lems_file,#_name, 
		                                           nogui=True, 
