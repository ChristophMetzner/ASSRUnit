    ...
    if entrytype == "Full":
        if not studies:  # full overview!
            if not observations:  # full overview!
                ...
                for k in keys:
                    ...
                    rowLabels = list(
                        set(rowLabels + des)
                    )  # this creates a list of unique(!) observations
                    ...
                    for j, o in enumerate(observations):
                        if o in database[k][entrytype].keys():
                            ...
                            for t in range(len(groups)):
                                string = (
                                    string + str(groups[t]) + " : " + str(vals[t]) + " "
                                )
                            values[j][i] = string
                    if meta:
                        print k
        ...
        if not studies:  # full overview!
            ...
            if not observations:  # full overview!
                ...
                ncols = len(colLabels)
                values = [
                    ["not tested" for _ in range(ncols)] for _ in range(nrows)
